...
export const ImageGenerator: React.FC<ImageGeneratorProps> = ({ savedAvatars, initialAvatar, userEmail }) => {
  ...
  const [status, setStatus] = useState<GenerationStatus>({ generating: false, message: '', progress: 0, error: null, phase: 'idle' });
  const [dragOverAvatarId, setDragOverAvatarId] = useState<string | null>(null);
  ...
  const processFile = useCallback(async (avatarId: string, file: File) => {
      if (file && file.type.startsWith('image/')) {
          const reader = new FileReader();
          reader.onloadend = async () => {
              try {
                  const jpegData = await transcodeToJpeg(reader.result as string);
                  setCharacterAssets(prev => ({ ...prev, [avatarId]: jpegData }));
              } catch (e) {
  ...
  const handleAssetUpload = (avatarId: string, e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
          processFile(avatarId, file);
      }
  };

  const handleDragEnter = (avatarId: string, e: React.DragEvent) => {
      e.preventDefault();
      e.stopPropagation();
      setDragOverAvatarId(avatarId);
  };

  const handleDragOver = (avatarId: string, e: React.DragEvent) => {
      e.preventDefault();
      e.stopPropagation();
      if (dragOverAvatarId !== avatarId) setDragOverAvatarId(avatarId);
  ...
  const handleDragLeave = (e: React.DragEvent) => {
      e.preventDefault();
      e.stopPropagation();
      setDragOverAvatarId(null);
  };

  const handleDrop = (avatarId: string, e: React.DragEvent) => {
      e.preventDefault();
      e.stopPropagation();
      setDragOverAvatarId(null);
  ...
  const handleGenerateVisual = async () => {
    ...
    setStatus({ generating: true, message: `Synthesizing Identity DNA for ${activeAvatars.length} characters...`, progress: 5, error: null, phase: 'image' });
    ...
    try {
        ...
        setStatus({ generating: false, message: 'Identity Locked.', progress: 100, error: null, phase: 'idle' });
    } catch (error: any) {
        setStatus({ generating: false, message: '', progress: 0, error: error.message || "An unexpected error occurred.", phase: 'idle' });
    }
  };
                ...
                <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {savedAvatars.map(avatar => {
                            ...
                            // Fix: Use avatar.image instead of characterSheet
                            const thumb = avatar.image;
                            const asset = characterAssets[avatar.id];
                            const isDragging = dragOverAvatarId === avatar.id;
                                    ...
                                    {isSelected && (
                                        <div className="space-y-2 pt-2 border-t border-slate-700/50">
                                            ...
                                            {asset ? (
                                                ...
                                                <label 
                                                    onDragEnter={(e) => handleDragEnter(avatar.id, e)}
                                                    onDragOver={(e) => handleDragOver(avatar.id, e)}
                                                    onDragLeave={handleDragLeave}
                                                    onDrop={(e) => handleDrop(avatar.id, e)}
                                                    className={`w-full h-20 rounded-xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition-all duration-200
                                                        ${isDragging 
                                                            ? 'border-cyan-400 bg-cyan-500/20 scale-[1.02] shadow-[0_0_15px_rgba(6,182,212,0.3)]' 
                                                            : 'border-slate-700/50 hover:border-cyan-500/50 hover:bg-cyan-500/5'
                                                        }`}
                                                >
                                                    <div className="pointer-events-none flex flex-col items-center">
                                                        <svg className={`w-4 h-4 mb-1 transition-colors ${isDragging ? 'text-cyan-400' : 'text-slate-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
                                                        <span className={`text-[8px] font-black uppercase tracking-widest transition-colors ${isDragging ? 'text-cyan-400' : 'text-slate-600'}`}>
                                                            {isDragging ? 'Drop Image' : 'Attach'}
                                                        </span>
                                                    </div>
